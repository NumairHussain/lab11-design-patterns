...
to the _operate method in engine_base class (line 54)
to the __operate_on_text method in engine_base (line 86)
to the create_operator_class in operators_factory (line 133)
```
...
Since they all share the same interface, each operator defines a specific anonymizing behavior and can be called via operator calls.
